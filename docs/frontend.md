...
| `SettingsAccount*`        | Account tab: `Profile` (display name; email is read-only, changed by an admin from Users), `Password` (requires the current one, optional "sign out everywhere else"), `Sessions` (device list with per-row and bulk revoke) |
| `SettingsUsers`           | Admin-only. Create an account, edit name/email (own row included), toggle role, ban/unban, set someone's password. The last admin cannot be demoted or banned, and nobody can ban themselves                                 |
