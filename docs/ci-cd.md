...
::: warning Cache budget
GitHub gives each repository a **10 GB** Actions cache with LRU eviction. Because the primary key includes `github.sha`, every run writes a new entry and older ones are evicted over time. That is fine — the `restore-keys` fallbacks still produce hits — but if cache hit rates look poor, this key scheme is the first thing to examine.
:::
...
::: warning Required status checks and the version PR
The version PR is opened using `GITHUB_TOKEN`. GitHub does not trigger workflow runs from events generated by `GITHUB_TOKEN` (a recursion guard), so **`ci.yml` does not run on the version PR**.

If you add _required_ status checks to the `main` branch protection rule, the version PR becomes permanently unmergeable — the required checks never start, and GitHub waits forever for a status that will never report. Resolve it by not requiring checks, using admin bypass for that PR, or giving `changesets/action` a PAT / GitHub App token so its PR triggers CI normally.

The image build is unaffected either way: a human-initiated merge produces a normal push event.
:::
