...
| `LOG_LEVEL`                      | No       | `info`         | Minimum log level: `trace`, `debug`, `info`, `warn`, `error`, `fatal`. Gates the production Pino transport only. Development uses the interactive pretty-terminal transport; `NODE_ENV=test` and `E2E_TEST=1` disable logging.                     |
