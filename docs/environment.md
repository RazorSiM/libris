...
| `TRUST_PROXY_HEADERS`            | No          | `0`            | Set to `1` to accept forwarded client-IP headers and the forwarded scheme (`x-forwarded-proto`, used in origin checks) only when the immediate TCP peer matches `LIBRIS_TRUSTED_PROXIES`.                                                                                                                                                                                                                                                                                                     |
...
| `general`     | every path except Better Auth's separately limited `/api/auth/*` — `/api/health` and `/api/health/live` included |
