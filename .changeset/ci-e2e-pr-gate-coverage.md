---
"@libris/api-hono": patch
"@libris/web": patch
---

Make the PR gate actually gate the authentication work, and run the production
config in CI at all.

Two structural gaps left this branch's security work unverified. The `e2e` job
runs `--grep @smoke` on pull requests, and `account.spec.ts`,
`isolation.spec.ts`, `first-run-setup.spec.ts`, `websocket-events.spec.ts` and
every `auth.spec.ts` block except `sign-in` carried no tag — so app passwords,
app-password scoping, OPDS Basic auth, the admin user-management walk and the
last-admin 409 first executed _after_ merge. Every block that pins an
authentication, authorization, ownership or session invariant is now tagged
`@smoke`; presentation coverage stays untagged so the gate stays fast. The PR
selection goes from 89 tests to 136.

`first-run-setup.spec.ts` is the one exception, and deliberately so: its
Playwright project depends on `chromium` to force it last, and Playwright does
not apply `--grep` to a dependency project — tagging it drags the entire suite
into every PR run. That is documented where someone will hit it.

Separately, every E2E run booted the API with `NODE_ENV=development`, because
`bootstrap.ts` refuses `E2E_TEST=1` in production and the suite needs the
`/__test/*` support routes that switch mounts. The development side of every
`NODE_ENV` branch was therefore the only side ever exercised. A new
`e2e-prod-config` job runs `NODE_ENV=production` with no `E2E_TEST` and no
`TEST_ROUTE_TOKEN`, over first-run setup, sign-in, sign-out and session
revocation, on its own database and Redis. It is the only place
`BETTER_AUTH_URL` is exercised: `trustedOrigins` is empty in production and
Better Auth resolves its trusted origin from `baseURL` once at startup, so a
wrong or missing value 403s every cookie-bearing POST rather than failing to
boot.

`docs/testing.md` now states which config branches CI covers and which it does
not, so the gap is readable rather than re-derived.
